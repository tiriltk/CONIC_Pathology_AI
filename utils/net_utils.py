...
def layer_normalize(AA, eps: float = 1e-5):
    batch_size, channel_size, height, width = AA.size(0), AA.size(1), AA.size(2), AA.size(3)

    AA = AA.view(AA.size(0), AA.size(1), -1)
    AA = AA - AA.min(2, keepdim=True)[0]
    AA = AA / (AA.max(2, keepdim=True)[0] + eps)
    AA = AA.view(batch_size, channel_size, height, width)
    AA = AA * 2 - 1
