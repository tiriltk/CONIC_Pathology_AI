...
"""
Co-registration following Matlab code and OpenCv examples. Fixed image is Visium and moving image is sample used for inference. 
Calculates the scale factor and scales the moving image to the fixed. Apply manual rotation and translation.
...
save_dir = "/Volumes/Expansion/biopsy_results/pannuke/40x/co_registration_final/" #Save path
...
def fine_co_reg(fixed_rgb, moving_manual_rgb):
    ...
    orb = cv2.ORB_create(5000) #ORB feature detector 
    kp1, des1 = orb.detectAndCompute(moving, None) #Find keypoints (kp) and descriptors (des)
    kp2, des2 = orb.detectAndCompute(fixed, None)
    feature_matcher = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=True) #Feature matcher 
    matches = feature_matcher.match(des1, des2) #Match the descriptors
    sorted_matches = sorted(matches, key=lambda x: x.distance) #Sort matches from distances
    selected_matches = sorted_matches[:500] #Selected the best matches
    ...
    for i, m in enumerate(selected_matches):
        moving_points[i] = kp1[m.queryIdx].pt #Get the coordinates of matched points in moving
        fixed_points[i] = kp2[m.trainIdx].pt #Get the coordinates of matched points in fixed

    matrix, inliers = cv2.estimateAffine2D(moving_points, fixed_points, cv2.RANSAC) #Estimate affine transformation, best results with estimateAffine2D
    
    matrix_resized = matrix.copy() 
    matrix_resized[0, 2] /=file_reduction #Same as used in Matlab to resize back translation
    matrix_resized[1, 2] /=file_reduction
    height, width = fixed_rgb.shape[:2]
    registered_image = cv2.warpAffine(moving_manual_rgb, matrix_resized, (width, height)) #Apply affine transformation to align moving image to fixed image
    
    return registered_image, matrix_resized

fixed_image = cv2.imread(fixed_path) #Load images
...
manual_aligned_image = manual_rotation_translation(moving_resized, 8, -100, 0) #Func116 sample manual parameters: Rotate 8 degrees, move -100 pixels in x, move 0 pixels in y
...
moving_fine_registered, affine_matrix = fine_co_reg(fixed_rgb, manual_aligned_image) #Fine adjustments
