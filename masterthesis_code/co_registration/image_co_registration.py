...
"""
Use the fixed Visium image and moving image from HoVerNet results. 
...
Fine adjustments and alignments. Plots and saves the results.
"""
...
def fine_co_reg(fixed_rgb, moving_rgb):
    ...
    #Match features between the two imagee
    matcher = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=True) #Brute force matcher with Hamming distance as measurement
    matches = matcher.match(des1, des2) #Match the sets of desciptors
    matches = sorted(matches, key=lambda x: x.distance) #Sort the matches from the Hamming distance
...
for name in (name_list):
    ...
    #OpenCV uses BGR. Need RGB for matplotlib
