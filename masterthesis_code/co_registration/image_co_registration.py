...
def fine_co_reg(fixed_rgb, moving_manual_rgb):
    ...
    orb = cv2.ORB_create(5000) #ORB detector, tried different values for feaures
    ...
    registered_image = cv2.warpAffine(moving_manual_rgb, matrix_resized, (width, height)) #Using matrix to transform moving image to fixed image

    affine_transform_matrix = np.eye(3, dtype=np.float32) #As done in matlab for saving affin transform matrix 3x3
