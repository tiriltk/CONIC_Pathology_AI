...
for name in (name_list):
    ...
    save_dir = "/Volumes/Expansion/biopsy_results/pannuke/40x/co_reg_opencv/"
    os.makedirs(save_dir, exist_ok=True)

    # 1) Lagre grove overlay
    ...
    # 2) Lagre finjustert (ORB/affine) overlay
    plt.imsave(os.path.join(save_dir, f"{name}_overlay_fine.png"), overlay_fine)

    # 3) Lagre det registrerte bildet alene (moving -> alignet til fixed)
    # reg_image er RGB (fra cv2.addWeighted / warpAffine på RGB)
    plt.imsave(os.path.join(save_dir, f"{name}_moving_registered.png"), reg_image)

    # 4) Lagre rotasjons-/affinmatrisen
    np.save(os.path.join(save_dir, f"{name}_rot_matrix.npy"), rot_matrix)

    # Lagre rotating_image som PNG
