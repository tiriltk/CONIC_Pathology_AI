...
class MSGEEightLoss(_Loss):
    ...
    """
    def __init__(self):
        super(MSGEEightLoss, self).__init__()
    ...
    def forward(self, true: torch.Tensor, pred: torch.Tensor, np_map_gt: torch.Tensor, device: int) -> torch.Tensor:
        ...
        def get_gradient_hv(hv):
            ...
            h_ch_neg = hv[..., 4].unsqueeze(1)  # Nx1xHxW
            v_ch_neg = hv[..., 5].unsqueeze(1)  # Nx1xHxW
            ch_45_neg = hv[..., 6].unsqueeze(1)
            ch_135_neg = hv[..., 7].unsqueeze(1)
            ...
            h_dh_ch_neg = F.conv2d(h_ch_neg, -kernel_h, padding=2)
            v_dv_ch_neg = F.conv2d(v_ch_neg, -kernel_v, padding=2)
            ch_dh_45_neg = F.conv2d(ch_45_neg, -kernel_45, padding=2)
            ch_dh_135_neg = F.conv2d(ch_135_neg, -kernel_135, padding=2)
            

            dhv = torch.cat([h_dh_ch, v_dv_ch, ch_dh_45, ch_dh_135, h_dh_ch_neg, v_dv_ch_neg, ch_dh_45_neg, ch_dh_135_neg], dim=1)
        ...
        focus = torch.cat([focus, focus, focus, focus, focus, focus, focus, focus], axis=-1)
