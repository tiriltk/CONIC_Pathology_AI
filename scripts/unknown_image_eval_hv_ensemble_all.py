...
def eval_models(log_name, FOLD_IDX, imgs_load_array, tp_num, exp_name0, encoder_name0, exp_name1, encoder_name1, epoch_idx=30):
    print("entered eval_models")
    # splits = joblib.load("/media/jenny/PRIVATE_USB/AugHoverData/splits_10/splits_10_per_dataset.dat")
    ...
    print("next9")
    imgs_valid = imgs_load_array[valid_indices] #dim for chosen images?
    print("next10")
    print(f"valid_indices: {valid_indices}")
    print(type(valid_indices))  # Should be a list or an array
    print(type(imgs_valid))     # Should be a list or an array
    ...
    #         nuclei_counts_df_gt, nuclei_counts_df_pred, f"/media/jenny/PRIVATE_USB/pannuke/pannuke_images/pannuke_test_results/{log_name}/{exp_name0}/{FOLD_IDX:02d}", epoch_idx, num_types=tp_num)
    
    output_dir = "/media/jenny/PRIVATE_USB/warwick_colab_results/Image_HE_MM218_C_300125_20x_BF_01/patches/20x/aug_hovernet/256x256_img_68_from_1024x1024"
    ...
    visualize_no_gt(imgs=imgs_load_array, pred=labels_array_pred , output_dir=output_dir)
...
if __name__ == "__main__":
    ...
    # img_path = "/media/jenny/PRIVATE_USB/AugHoverData/data/images.npy"
    # ann_path = "/media/jenny/PRIVATE_USB/AugHoverData/data/labels.npy"
    # labels = np.load(ann_path)
    # imgs_load = np.load(img_path)
    tile_path = "/media/jenny/PRIVATE_USB/warwick_colab_patches/HE_MM218_C_300125_20x_BF_01/20x/256x256_img_68_from_1024x1024/" 
    ...
    for i in range(len(image_list)):
        ...
        print(f"{len(imgs_load)}")
        # If your image is RGBA or other formats, you might want to convert it to RGB or grayscale
        # For example, to convert to RGB:
        imgs_load = np.array(image.convert("RGB"))
        imgs_load_list.append(imgs_load)
        print(f"Loaded image shape: {imgs_load.shape}")  # Print the shape of the loaded image
    imgs_load_array = np.array(imgs_load_list)
    print(imgs_load_array)
    folder_idx = int(args.folder_idx)
    
    epoch_idx = 49
    eval_models(args.log_name, folder_idx, imgs_load_array, 7, \
