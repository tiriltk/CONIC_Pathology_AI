...
from models.model_head_eight_axis import HoVerNetHeadExt
...
def eval_func(true_array, pred_array, true_csv, pred_csv, out_dir, epoch_idx, num_types=0):
    ...
    for patch_idx in tqdm(range(nr_patches)):
        ...
        mpq_info_single = get_multi_pq_info(true, pred, nr_classes=num_types)
        mpq_info = []
...
def eval_models(FOLD_IDX, imgs_load, labels, tp_num, exp_name, encoder_name, epoch_idx=30):
    ...
    # os.makedirs(f"wenhua_docker_test/{args.exp_name}/{FOLD_IDX:02d}", exist_ok=True)
    # np.save(f"wenhua_docker_test/{args.exp_name}/{FOLD_IDX:02d}/predictions.npy", labels_array_pred) 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_name', type=str, default='hover_paper_eight_axis_conic')
    ...
    for FOLD_IDX in fold_idxes:
        folder_idx = int(FOLD_IDX)
